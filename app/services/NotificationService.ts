...
class NotificationService {
    ...
    private systemNotificationInterval: any = null;
    ...
    // Start sending system notifications every 5 minutes
    startSystemNotifications() {
        if (this.systemNotificationInterval) return; // Prevent multiple intervals
        this.systemNotificationInterval = setInterval(() => {
    ...
    stopSystemNotifications() {
        if (this.systemNotificationInterval) {
            clearInterval(this.systemNotificationInterval);
            this.systemNotificationInterval = null;
        }
    }
